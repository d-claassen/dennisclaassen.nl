 ...
 * @subpackage LinkedIn
 */
class Application_Service_LinkedIn extends Zend_Rest_Client
{
	/**
	 * Base URL for API
	 */
	...
	public function  __construct($options = null, $consumer = null)
	{
		...
		if ($consumer instanceof Zend_Oauth_Consumer) {
			...
			$this->_oauthConsumer = new Zend_Oauth_Consumer($this->_options);
		}
				
		if (isset($this->_options['accessToken'])) {
			// Get http client from accesstoken (if provided)
			$this->setLocalHttpClient($this->_options['accessToken']->getHttpClient($this->_options));
		} else {
	...
	public function userProfile($user = null, $includeCustomFields = false)
	{
		...
		if ($includeCustomFields) {
			$fields = array('first-name', 'last-name', 'positions', 'twitter-accounts', 'headline', 'summary', 'picture-url', 'skills', 'educations');
			$path .= ':(' . implode(',', $fields) . ')';
		}
  ...
  public function company( $id = null, $includeCustomFields = false )
  {
    $path = '/v1/companies/' . $id;

    if ($includeCustomFields) {
      $fields = array( 'id', 'name', 'website-url', 'logo-url', 'description', 'twitter-id', 'locations' );
      $path .= ':(' . implode(',', $fields) . ')';
    }
