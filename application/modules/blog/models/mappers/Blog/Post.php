...
class Blog_Model_Mapper_Blog_Post extends Application_Model_Mapper_Abstract
{   
    ...
    public function fetchBy( $fields )
    {
        ...
        foreach ($data as $row)
        {
            $entry = new Blog_Model_Blog_Post($row);
            $entries[] = $entry;
        }
        return $entries;
    }
    ...
    public function fetchAll()
    {
        ...
        foreach ($resultSet as $row) {
            $entry = new Blog_Model_Blog_Post();
    ...
    public function getBlogsByLabel( $label_id )
    {
    	...
    	$select = $this->getDbTable()->select()->setIntegrityCheck( false );
    	$select->from( array( 'b' => $this->getDbTable()->info( 'name' ) ) )
    		->join( array( 'bl' => 'blog_post_label' ), 'bl.blog_id = b.id', array() )
